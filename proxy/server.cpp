                ...
                switch (packet_type) {
                    ...
                    case NET_MESSAGE_GAME_PACKET: {
                        ...
                        switch (packet->m_type) {
                            case 3: {
                                auto lalal = utils::GetStructPointerFromTankPacket(evt.packet);
                                PlayerMovings* data = unpackRaw(utils::GetStructPointerFromTankPacket(evt.packet));
                                if (gt::placetp) {
                                    int punchx;
                                    int punchy;
                ...
                if (event.packet->data) {
                    ...
                    switch (packet_type) {
                        ...
                        case NET_MESSAGE_GAME_PACKET: {
                            ...
                            switch (packet->m_type) {
                            case 25: {
                                gt::send_log("Ignoring Packet Type 25");
                            } break;
                            case 3: {
                                auto data = utils::GetStructPointerFromTankPacket(event.packet);
                                int punchx;
                                int punchy;
                                memcpy(&punchx, data + 44, 4);
                                memcpy(&punchy, data + 48, 4);
                                //std::string log = "PosX: " + std::to_string(punchx) + " PosY: " + std::to_string(punchy);
                                //gt::send_log(log);
                            } break;
                                case 8: {
                                    if (!packet->m_int_data) {
                                        std::string dice_roll = std::to_string(packet->m_count + 1);
                                        gt::send_log("The Property Will Roll `9" + dice_roll);
                                    }
                            } break;           
                                case PACKET_PING_REQUEST: {
                                    ...
                                    gt::send_log("Ping Sended. Avoid Disconnect ~ Banned");
                            } break;
                                case PACKET_CALL_FUNCTION:
